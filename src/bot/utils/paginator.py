...
RIGHT_ARROW_EMOJI = '<:rightArrow:1191861189202956328>'
LEFT_ARROW_EMOJI = '<:leftArrow:1191861332736213143>'
DELETE_RESPONSE_EMOJI = '<:deleteResponse:1191861348754276432>'
SEARCH_ICON_EMOJI = '<:searchIcon:1191861363493052536>'
CUSTOM_PAGE_EMOJI = '<:customPage:1191949336951672882>'
...
class RACPages(discord.ui.View):
    def __init__(
        self,
    ...
    async def start(self, *, content: Optional[str] = None, reply: bool = True, ephemeral: bool = False) -> None:
        if self.check_embeds and not self.ctx.channel.permissions_for(self.ctx.me).embed_links:  # type: ignore
            await self.ctx.send('⚠ The bot requires `Embed Links` for this to work.', ephemeral=True)
            return
        ...
        if reply:
            self.message = await self.ctx.reply(**kwargs, view=self, ephemeral=ephemeral)
        else:
            self.message = await self.ctx.send(**kwargs, view=self, ephemeral=ephemeral)

    @discord.ui.button(emoji=LEFT_ARROW_EMOJI, style=discord.ButtonStyle.grey) # label='≪',
    ...
    @discord.ui.button(emoji=RIGHT_ARROW_EMOJI, style=discord.ButtonStyle.grey) # label='≫',
    async def go_to_last_page(self, interaction: discord.Interaction, button: discord.ui.Button):
        """go to the last page"""
        # The call here is safe because it's guarded by skip_if
        await self.show_page(interaction, self.source.get_max_pages() - 1)  # type: ignore

    @discord.ui.button(emoji=CUSTOM_PAGE_EMOJI, style=discord.ButtonStyle.grey) # label='Skip to page...',
    ...
    @discord.ui.button(emoji=DELETE_RESPONSE_EMOJI, style=discord.ButtonStyle.red) # label='Quit', 
...
class FieldPageSource(menus.ListPageSource):
    def __init__(
        ...
        *,
        ...
        self.embed: discord.Embed = discord.Embed(colour=discord.Colour.from_rgb(133, 133, 133)) # .blurple()
        self.clear_description: bool = clear_description
        self.inline: bool = inline

    async def format_page(self, menu: RACPages, entries: list[tuple[Any, Any]]) -> discord.Embed:
...
class SimplePages(RACPages):
    def __init__(self, entries, *, ctx: Union[Context, GuildContext], per_page: int = 12):
        super().__init__(SimplePageSource(entries, per_page=per_page), ctx=ctx)
        self.embed = discord.Embed(colour=discord.Colour.from_rgb(133, 133, 133))